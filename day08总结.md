...
## 错误用法
- 我想让光源随着时间 sin 做彩色变换，我用了 t = glfwGetTime，直接shader fragColor * t
  - 第一个问题：导致了输出光源黑色。 因为 fragColor 是输出字段，肯定都是 000 值。
  - 第二个问题：使用 vec4(1.0) * t,导致光源不是预期颜色，只是白灰黑三种变换。 因为 rgb 是同阶，rgb 改为三个不同弧度值就行。
- 我想光源绕被照射的物体旋转，用 T(p)*R(p)*T(-p)
  - 但是预期效果不对，因为写是从上到下，但是真实执行是从下到上的，简称右乘